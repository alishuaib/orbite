...
class section{
    public static function builder(object $section){
        return new _schema($section);
    }

    public static function patch(_schema $body){
        $response = \local_orbite\api::post_request("/section?method=PATCH", $body -> read());
        return $response;
    }

    public static function delete(string $id){
        $response = \local_orbite\api::post_request("/section?method=DELETE", array(
            'section' => array(
                'id'=>[(int)$id],
            ),
        ), []);
